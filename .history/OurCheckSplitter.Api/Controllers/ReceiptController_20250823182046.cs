...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class ReceiptController : ControllerBase
    {
        ...
        private async Task UpdateFriendReceiptRelationships(int receiptId)
        {
            ...
            // Get all friends that are directly assigned to this receipt
            var friendsFromDirectAssignment = await _context.FriendReceipts
            ...
            var allFriendsInReceipt = friendsFromItems.Union(friendsFromDirectAssignment).Distinct().ToList();

            Console.WriteLine($"Found {friendsFromItems.Count} friends from items, {friendsFromDirectAssignment.Count} friends from direct assignment, total: {allFriendsInReceipt.Count}");
            ...
            Console.WriteLine($"Removed {existingFriendReceipts.Count} existing FriendReceipt relationships for receipt {receiptId}");

            // Create new FriendReceipt relationships for all friends
            foreach (var friendId in allFriendsInReceipt)
            {
            ...
            [FromQuery] string? searchTerm = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                ...
                query = query.Where(r =>
                    ...
                    r.FriendReceipts!.Any(fr => fr.Friend.Name != null && fr.Friend.Name.ToLower().Contains(searchTermLower)));
            }
        ...
        [HttpPost("fix-all-relationships")]
        public async Task<IActionResult> FixAllReceiptRelationships()
        {
            var user = HttpContext.Items["User"] as AppUser;
            if (user == null)
                return Unauthorized();

            // Get all receipts for this user that have items with friend assignments
            var receiptsToFix = await _context.Receipts
                .Where(r => r.UserId == user.Id)
                .Where(r => r.Items.Any(i => i.Assignments.Any(a => a.FriendAssignments.Any())))
                .Select(r => r.Id)
                .ToListAsync();

            var fixedCount = 0;
            foreach (var receiptId in receiptsToFix)
            {
                await UpdateFriendReceiptRelationships(receiptId);
                fixedCount++;
            }
            ...
            return Ok(new
            {
                message = $"Fixed relationships for {fixedCount} receipts",
                receiptsFixed = fixedCount
            });
        }
