...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class ReceiptController : ControllerBase
    {
        ...
        // Helper method to update the direct relationship between friends and receipts
        private async Task UpdateFriendReceiptRelationships(int receiptId)
        {
            ...
            // Update the direct relationship for these friends
            foreach (var friendId in friendsInReceipt)
            {
                var friend = await _context.Friends.FindAsync(friendId);
                if (friend != null && friend.ReceiptId != receiptId)
                {
                    friend.ReceiptId = receiptId;
                }
            }

            // For friends that are no longer assigned to any items in this receipt,
            // clear their ReceiptId if it was set to this receipt
            var friendsToClear = await _context.Friends
                .Where(f => f.ReceiptId == receiptId &&
                           !friendsInReceipt.Contains(f.Id))
                .ToListAsync();

            foreach (var friend in friendsToClear)
            {
        ...
        public async Task<IActionResult> AssignFriendsToItems([FromBody] AssignFriendsToItemDto dto)
        {
            ...
            // Update the direct relationship between friends and receipt
            foreach (var friendId in friendsToAssign)
            {
                var friend = await _context.Friends.FindAsync(friendId);
                if (friend != null && friend.ReceiptId != receipt.Id)
                {
                    friend.ReceiptId = receipt.Id;
                }
            }
