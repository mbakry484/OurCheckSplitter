...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class FinalAmountController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetFriendAmounts(int receiptId)
        {
            ...
            // Add tips based on whether they are included in total or not
            if (friendCount > 0)
            {
                if (!tipsIncluded)
                {
                    // Tips not included: distribute equally among friends
                ...
                {
                    // Tips included: distribute proportionally based on each friend's share
                    var totalBeforeTips = result.Sum(f => f.AmountToPay ?? 0);
                    if (totalBeforeTips > 0)
                    {
                        foreach (var friendResult in result)
                        {
                            var friendShare = (friendResult.AmountToPay ?? 0) / totalBeforeTips;
                            var friendTips = totalTips * friendShare;
                            friendResult.AmountToPay = Math.Round((friendResult.AmountToPay ?? 0) + friendTips, 2);
                        }
                    }
                }
            }
