    ...
    public class TestController : ControllerBase
    {
        ...
        public async Task<IActionResult> DebugReceiptCreation([FromBody] ReceiptDto dto)
        {
            ...
            try
            {
                ...
                try
                {
                    var directQuery = await _context.Database.SqlQueryRaw<dynamic>(
                        "SELECT Id, Name, Tax, TaxType, Tips, TipsIncludedInTotal, Total FROM Receipts WHERE Id = {0}",
                        receipt.Id
                    ).ToListAsync();

                    if (directQuery.Any())
                    {
                        var rawData = directQuery.First();
                        Console.WriteLine($"Direct SQL Query: Id={rawData.Id}, TaxType={rawData.TaxType}, TipsIncludedInTotal={rawData.TipsIncludedInTotal}");
                    }
                }
                catch (Exception sqlEx)
                {
                    Console.WriteLine($"Direct SQL query failed: {sqlEx.Message}");
                }
