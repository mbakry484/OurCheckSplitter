...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class FinalAmountController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetFriendAmounts(int receiptId)
        {
            ...
            // Calculate the expected receipt total for validation (excluding tips if not included)
            var expectedReceiptTotal = tipsIncluded ? (decimal)receipt.Total : (decimal)receipt.Total;
            ...
            // NOW add tips separately if not included in total (after validation)
