...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class FinalAmountController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetFriendAmounts(int receiptId)
        {
            ...
            // Calculate subtotal based on whether tips are included in total
            if (tipsIncluded)
            {
                // Tips are included in total, so subtract them to get subtotal
                subtotal = (decimal)receipt.Total - (decimal)receipt.Tax - totalTips;
            }
            else
            {
                // Tips are NOT included in total, so only subtract tax
            ...
            // Add tips if included in total (when tipsIncludedInTotal is true)
            // When tipsIncludedInTotal is false, tips are not part of the receipt total
            // so we don't add them to friend amounts
