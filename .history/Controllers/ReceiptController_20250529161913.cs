...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class ReceiptController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetFriendAmounts(int receiptId)
        {
            ...
            // Ensure the sum does not exceed the receipt total
            var totalCalculated = result.Sum(f => f.AmountToPay ?? 0);
            var receiptTotal = (decimal)receipt.Total;
            if (totalCalculated > 0 && totalCalculated > receiptTotal)
            {
                var scale = receiptTotal / totalCalculated;
                foreach (var friend in result)
                {
                    friend.AmountToPay = Math.Round((friend.AmountToPay ?? 0) * scale, 2);
                }
            }
