...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class ReceiptController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllReceipts()
        {
            ...
            {
                var receiptDto = _mapper.Map<ReceiptResponseDto>(receipt);
                foreach (var item in receipt.Items)
                {
                    var itemDto = receiptDto.Items.First(i => i.Id == item.Id);
                    var assignments = item.Assignments.OrderBy(a => a.Id).ToList();
                    for (int i = 0; i < assignments.Count; i++)
                    {
                        var assignment = assignments[i];
                        var assignmentDto = itemDto.Assignments.First(a => a.Id == assignment.Id);
                        assignmentDto.UnitNumber = i + 1;
                        assignmentDto.Unitlabel = $"unit{i + 1}";
                    }
                }
