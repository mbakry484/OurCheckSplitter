...
namespace OurCheckSplitter.Api.Controllers
{
    ...
    public class ReceiptController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteReceipt(int id)
        {
            ...
            // Remove all related FriendAssignments, ItemAssignments, and Items
            foreach (var item in receipt.Items)
            {
                foreach (var assignment in item.Assignments)
                {
                    _context.FriendAssignments.RemoveRange(assignment.FriendAssignments);
                }
                _context.ItemAssignments.RemoveRange(item.Assignments);
            }
            _context.Items.RemoveRange(receipt.Items);

            // Now remove the receipt itself
