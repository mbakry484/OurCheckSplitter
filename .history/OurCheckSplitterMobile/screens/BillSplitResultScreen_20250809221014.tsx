...
const BillSplitResultScreen = ({ navigation, route }: BillSplitResultScreenProps) => {
  ...
  // Estimate how many text lines will be rendered (after wrapping) to inform scaling
  const estimatedLines = useMemo(() => {
    const maxChars = 48;
    let lines = 4; // title + date + separator + total row
    friendBills.forEach((bill) => {
      lines += 1; // friend header line
  ...
  // Adaptive font scaling only:
  // - Upscale slightly when few friends so receipt feels full
  // - Downscale gently when content is dense to keep within shape
  const contentScale = useMemo(() => {
    if (friendCount <= 3) return 1.12;
    if (friendCount <= 5) return 1.04;
    const target = 26; // desired line count that fills the receipt height
    const s = target / Math.max(1, estimatedLines);
    return Math.max(0.75, Math.min(1, s));
