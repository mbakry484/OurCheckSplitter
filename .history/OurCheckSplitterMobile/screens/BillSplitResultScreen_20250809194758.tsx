...
const BillSplitResultScreen = ({ navigation, route }: BillSplitResultScreenProps) => {
  ...
  const renderFriendBill = (friendBill: FriendBill, index: number, totalFriends: number) => {
    // Dynamic sizing based on number of friends
    const dynamicCardStyle = {
      ...styles.friendBillCard,
      paddingVertical: totalFriends > 4 ? 8 : 12,
      marginBottom: totalFriends > 6 ? 6 : totalFriends > 4 ? 8 : 12,
    };
    
    const dynamicFontSize = {
      fontSize: totalFriends > 6 ? 14 : totalFriends > 4 ? 15 : 16,
    };
    
    const dynamicItemFontSize = {
      fontSize: totalFriends > 6 ? 11 : totalFriends > 4 ? 12 : 13,
    };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Receipt Summary - Fixed Size Container */}
      <View ref={receiptRef} style={styles.receiptContainer} collapsable={false}>
        ...
        <View style={styles.friendBillsSection}>
          ...
          <View style={[styles.friendBillsContainer, { maxHeight: screenDimensions.height * 0.5 }]}>
            {friendBills.map((friendBill, index) => renderFriendBill(friendBill, index, friendBills.length))}
          </View>
        </View>
      </View>
    </SafeAreaView>
