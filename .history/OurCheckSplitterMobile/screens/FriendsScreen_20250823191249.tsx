...
const FriendsScreen = ({ navigation, route }: FriendsScreenProps) => {
  ...
  const loadFriendReceipts = async (friendId: string) => {
    try {
      ...
      // Update the friend with receipt names (simple format)
      const receiptNames = friendDetails.receipts.map((r: any) => r.name || r.title || 'Unknown Receipt');
      setFriends(prev => 
        prev.map(f => 
          f.id === friendId 
            ? { ...f, receipts: receiptNames } 
            : f
        )
      );

      console.log(`Loaded ${receiptNames.length} receipts for friend ${friendId}`);
