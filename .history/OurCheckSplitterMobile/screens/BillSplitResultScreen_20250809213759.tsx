...
const BillSplitResultScreen = ({ navigation, route }: BillSplitResultScreenProps) => {
  ...
  const contentScale = useMemo(() => {
    ...
    const target = 22;
    const minScale = 0.6;
    const maxScale = 1.35; // allow scaling up to fill space
    if (totalLines <= 0) return 1;
    const ratio = target / totalLines;
    // If ratio > 1 → we have extra space → scale up (capped). If < 1 → scale down (floored).
    if (ratio >= 1) return Math.min(maxScale, ratio);
  ...
  const renderFriendsReceipt = () => (
    <View style={styles.page}>
      ...
      <View ref={receiptRef} style={styles.receiptContainer} collapsable={false}>
        ...
        <View style={styles.receiptHeaderArea}>
          {!!receiptData?.receiptTitle && (
            <View style={styles.titleOverlayRow}>
              <Text style={[styles.receiptTitle, { fontSize: Math.round(22 * contentScale) }]}>
                {receiptData.receiptTitle}
              </Text>
              {!isSharing && (
                <TouchableOpacity
          ...
          <Text style={[styles.receiptDate, { fontSize: Math.round(14 * contentScale) }]}>
            {receiptData?.receiptDate || ''}
          </Text>
          <View style={styles.separator} />
        </View>

        {/* Friend Blocks - pack content and rely on scaling */}
        <View style={styles.friendBlocks}>
          {friendBills.map((bill) => {
            const itemsInline = bill.items
              ...
              .join(', ');
            return (
              <View key={bill.friend.id} style={[styles.friendBlock, { marginBottom: Math.round(6 * contentScale) }]}>
                <View style={styles.friendRow}>
                  <Text style={[styles.friendNameBW, { fontSize: Math.round(18 * contentScale) }]}>
                    {bill.friend.name}
                  </Text>
                  <Text style={[styles.friendAmountBW, { fontSize: Math.round(18 * contentScale) }]}>
                    ${bill.totalAmount.toFixed(2)}
                  </Text>
                </View>
                {itemsInline.length > 0 && (
                  <Text style={[styles.itemsInlineBW, { fontSize: Math.round(14 * contentScale), lineHeight: Math.round(18 * contentScale) }]}>
                    {itemsInline}
                  </Text>
                )}
        ...
        {/* Flexible spacer to push total to the bottom without adding gaps between friends */}
        <View style={{ flexGrow: 1 }} />
        {/* Grand Total */}
        <View style={styles.totalRowBW}>
          <Text style={[styles.totalLabelBW, { fontSize: Math.round(20 * contentScale) }]}>TOTAL</Text>
          <Text style={[styles.totalValueBW, { fontSize: Math.round(20 * contentScale) }]}>
  ...
  const renderOriginalReceipt = () => (
    <View style={styles.page}>
      <View style={styles.receiptContainer}>
        <View style={styles.receiptHeaderArea}>
          {!!receiptData?.receiptTitle && (
            <Text style={[styles.receiptTitle, { fontSize: Math.round(22 * itemsContentScale) }]}>
              {receiptData.receiptTitle}
            </Text>
          )}
          <Text style={[styles.receiptDate, { fontSize: Math.round(14 * itemsContentScale) }]}>
            {receiptData?.receiptDate || ''}
          </Text>
          <View style={styles.separator} />
        </View>
        ...
        <View style={styles.friendBlocks}>
          {(receiptData?.items || []).map((it: ReceiptItem, idx: number) => (
            <View key={it.id || idx} style={styles.friendBlock}>
              <View style={styles.friendRow}>
                <Text style={[styles.friendNameBW, { fontSize: Math.round(16 * itemsContentScale) }]}>
                  {it.name}
                </Text>
                <Text style={[styles.friendAmountBW, { fontSize: Math.round(16 * itemsContentScale) }]}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Horizontal pager with two receipt pages */}
      <ScrollView
        horizontal
        pagingEnabled
        showsHorizontalScrollIndicator={false}
        onMomentumScrollEnd={handleScrollEnd}
        contentContainerStyle={styles.pagerContainer}
      >
