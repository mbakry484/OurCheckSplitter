...
const HomeScreen = ({ navigation }: HomeScreenProps) => {
  ...
  const loadData = async (page: number = 1, isRefresh: boolean = false) => {
    try {
      ...
      if (receiptsResponse && Array.isArray(receiptsResponse)) {
        ...
        // Paginated format
        convertedReceipts = await Promise.all(
          receiptsResponse.items.map(async (receipt: ReceiptResponseDto) => {
            try {
              // Fetch friends for this specific receipt
              const receiptFriends = await api.receipts.getReceiptFriends(receipt.id);
              console.log(`Fetched friends for receipt ${receipt.id}:`, receiptFriends);
  ...
  const loadMoreReceipts = () => {
    if (receiptsPagination.hasNextPage && !loadingMore) {
      loadData(receiptsPagination.currentPage + 1, false);
    }
  };
