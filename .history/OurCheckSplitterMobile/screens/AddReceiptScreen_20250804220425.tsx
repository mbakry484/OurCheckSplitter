    ...
    return (
      <View key={item.id} style={styles.itemContainer}>
        <View style={styles.itemRow}>
          {/* Dropdown button for subitems when quantity > 1 */}
          {quantity > 1 && (
            <TouchableOpacity 
              onPress={() => {
                if (!hasSubitems) {
              ...
              style={styles.subitemDropdownButton}
            >
              <Ionicons 
        ...
        {/* Subitems Section - Only when splitEqually is OFF */}
        {isExpanded && hasSubitems && !item.splitEqually && (
        ...
        {/* Show friends list for items with quantity > 1 */}
        {quantity > 1 && (
          <View style={styles.itemFriendsListContainer}>
            <Text style={styles.itemFriendsListLabel}>Assigned Friends:</Text>
            {item.assignedFriends.length > 0 ? (
              <View style={styles.itemFriendsList}>
                {friends.filter(friend => item.assignedFriends.includes(friend.id)).map(friend => (
                  <View key={friend.id} style={styles.itemFriendTag}>
                    <Text style={styles.itemFriendAvatar}>{friend.avatar}</Text>
                    <Text style={styles.itemFriendName}>{friend.name}</Text>
                  </View>
                ))}
              </View>
            ) : (
              <Text style={styles.noFriendsAssigned}>No friends assigned</Text>
            )}
          </View>
        )}
        
        {/* Show only selected friends from top section for main item - only when quantity = 1 or not expanded */}
        {selectedFriends.length > 0 && quantity === 1 && (
