...
const FriendsScreen = ({ navigation, route }: FriendsScreenProps) => {
  ...
  const loadFriends = async (page: number = 1, isRefresh: boolean = false, searchTerm?: string) => {
    try {
      ...
      const mapFriendData = async (friend: any): Promise<Friend> => {
        // Map receipts with participants (limit to 5 most recent receipts to avoid too many API calls)
        const limitedReceipts = (friend.receipts || []).slice(0, 5);
        ...
        return {
          ...
          detailedReceipts: detailedReceipts
        };
      };
