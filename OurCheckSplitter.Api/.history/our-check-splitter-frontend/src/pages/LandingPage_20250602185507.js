    ...
    return (
        <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)' }}>
            ...
            <Box sx={{ p: 4 }}>
                ...
                <Grid container spacing={3}>
                    {dummyReceipts.map(receipt => (
                        <Grid item xs={12} sm={6} md={4} key={receipt.id}>
                            <Card sx={{ borderRadius: 3, boxShadow: 4, transition: '0.2s', '&:hover': { boxShadow: 8, transform: 'scale(1.03)' } }}>
                                <CardContent>
                ...
                {/* AddReceiptModal placeholder */}
                {/* {open && <AddReceiptModal open={open} onClose={() => setOpen(false)} />} */}
            </Box>
        </Box>
    );
};
