...
const AddReceiptScreen = ({ navigation, route, onEditBasicData }: AddReceiptScreenProps) => {
  ...
  // Debug logging
  console.log('AddReceiptScreen - isEditing:', isEditing);
  console.log('AddReceiptScreen - editingData:', editingData);
  console.log('AddReceiptScreen - basicData:', basicData);
  
  // Form state - pre-populate with basic data if available
  const [receiptTitle, setReceiptTitle] = useState(
    isEditing ? editingData?.receiptTitle : (basicData?.receiptName || '')
  );
  const [receiptDate, setReceiptDate] = useState(
    isEditing ? editingData?.receiptDate : (basicData?.date || new Date().toLocaleDateString())
  );
  const [items, setItems] = useState<ReceiptItem[]>(
    isEditing ? editingData?.items : [
      { id: '1', name: '', price: '', quantity: '1', assignedFriends: [], splitEqually: true, subitems: [] }
    ]
  );
  const [tip, setTip] = useState(
    isEditing ? editingData?.tip?.toString() : (basicData?.tips || '')
  );
  ...
  console.log('- receiptTitle:', receiptTitle);
  ...
  useEffect(() => {
    if (isEditing && editingData) {
      console.log('Updating state with editing data:', editingData);
      setReceiptTitle(editingData.receiptTitle || '');
      setReceiptDate(editingData.receiptDate || '');
  ...
  const handleSaveReceipt = async () => {
    ...
    try {
      ...
      // Step 3: Update the receipt's friend relationships to ensure friends appear in the receipt
      try {
        console.log('Updating receipt friend relationships...');
        // Get all unique friend names that were assigned to items
        const allAssignedFriendNames = new Set<string>();
        createdItems.forEach(item => {
          item.assignedFriends.forEach(friendId => {
            const friend = friends.find(f => f.id === friendId);
            if (friend) {
              allAssignedFriendNames.add(friend.name);
            }
          });
        });

        if (allAssignedFriendNames.size > 0) {
          await api.receipts.assignFriendsToReceipt(receiptId, {
            friendNames: Array.from(allAssignedFriendNames)
          });
          console.log(`Successfully updated receipt friend relationships for: ${Array.from(allAssignedFriendNames).join(', ')}`);
        }
      } catch (receiptUpdateError: any) {
        console.error('Failed to update receipt friend relationships:', receiptUpdateError);
        // Don't show error to user as this is a background sync operation
        console.log('Receipt saved but friend relationships may not be fully synced');
      }
    ...
    <SafeAreaView style={styles.container}>
       ...
       <View style={[styles.header, { paddingTop: Math.max(16, insets.top) }]}>
         ...
         <Text style={styles.headerTitle}>
           {isEditing ? 'Edit Receipt' : (basicData ? 'Add Items & Split' : 'Add Receipt')}
         </Text>
